...
class QuizEarTraining2 extends StatefulWidget {
  const QuizEarTraining2({Key? key}) : super(key: key);

  @override
  _QuizEarTraining2State createState() => _QuizEarTraining2State();
}

class _QuizEarTraining2State extends State<QuizEarTraining2> {
  int _score = 0;
  ...
  // List of all keys (white and black) to use in our quiz
...
Future<String> getUrlFromAsset(String assetPath) async {
  ...
  void _generateQuestions() {
    ...
    for (int i = 0; i < 10; i++) {
      // Select a correct answer from all keys
      final int correctNoteIndex = _random.nextInt(allKeys.length);
      final String correctNote = allKeys[correctNoteIndex];
      
      // Create wrong options by selecting three different notes
      final List<String> options = [correctNote];
      while (options.length < 4) {
        final String randomNote = allKeys[_random.nextInt(allKeys.length)];
  ...
  String _getScoreMessage() {
    if (_score == 10) {
      ...
      return 'Excellent job! You can identify both white and black keys with great accuracy!';
    } else if (_score >= 6) {
      return 'Good work! Keep practicing to improve your ability to identify all piano keys.';
    } else if (_score >= 4) {
      return 'You\'re making progress! Regular practice with black keys will help improve your skills.';
    } else {
      return 'Identifying all 12 notes is challenging! Keep practicing daily to train your ear.';
    }
  }
}
